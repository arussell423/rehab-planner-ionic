...
export class ScheduleService {
  ...
  deleteAllData(): void {
    const keys = this.storage.keys().filter(k => k.startsWith('rp_'));
  ...
  generateTherapistReport(profileName: string, phase: string): void {
    const schedule = this.getSchedule();
    const wdata = this.getWeekData();
    const dayNames = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
    const weekStart = this.getMondayOfWeek(new Date()).toLocaleDateString();

    let html = `<!DOCTYPE html><html><head><meta charset="utf-8">
  <title>Rehab Report — ${profileName}</title>
  <style>
  ...
  <button onclick="window.print()" style="float:right;padding:8px 20px;background:#6b9955;color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:0.9rem">🖨 Print</button>
  <h1>Weekly Rehab Report</h1>
  <p><strong>Patient:</strong> ${profileName} &nbsp;|&nbsp; <strong>Phase:</strong> ${phase} &nbsp;|&nbsp; <strong>Week of:</strong> ${weekStart}</p>
  <h2>Activity Completion</h2>
  <table><tr><th>Day</th><th>Status</th><th>Completed</th><th>Total</th><th>%</th><th>Pain</th><th>Steps</th></tr>`;

    schedule.forEach(s => {
      const d = wdata[s.day];
      if (s.rest) {
        html += `<tr><td>${s.day}</td><td><span class="badge green">Rest</span></td><td>—</td><td>—</td><td>—</td><td>—</td><td>—</td></tr>`;
        return;
      }
      const total = s.morning.length + s.afternoon.length + s.evening.length;
      const done = d ? Object.values(d.activities || {}).filter(Boolean).length : 0;
      const pct = total ? Math.round((done / total) * 100) : 0;
      const pain = d?.pain ?? '—';
      const steps = d?.steps || '—';
      const cls = pct >= 80 ? 'green' : pct >= 50 ? 'amber' : 'red';
      html += `<tr><td>${s.day}</td><td><span class="badge ${cls}">${pct >= 80 ? '✓ Good' : pct >= 50 ? '~ Partial' : '✗ Low'}</span></td><td>${done}</td><td>${total}</td><td>${pct}%</td><td>${pain}/10</td><td>${steps}</td></tr>`;
    });

    html += `</table><h2>Health Metrics (This Week)</h2><table><tr><th>Day</th><th>Sleep Hrs</th><th>Sleep Quality</th><th>Weight</th><th>Calories</th></tr>`;
    schedule.forEach(s => {
      const d = wdata[s.day];
      html += `<tr><td>${s.day}</td><td>${d?.sleepHrs || '—'}</td><td>${d?.sleep ? '★'.repeat(d.sleep) + '☆'.repeat(5 - d.sleep) : '—'}</td><td>${d?.weight ? d.weight + ' ' + (d.weightUnit || 'kg') : '—'}</td><td>${d?.calories || '—'}</td></tr>`;
    });

    html += `</table><h2>Therapist Notes</h2>`;
    let hasNotes = false;
    schedule.forEach(s => {
      const d = wdata[s.day];
      if (d?.therapistNotes) {
        html += `<p><strong>${s.day}:</strong></p><div class="notes">${d.therapistNotes}</div>`;
        hasNotes = true;
      }
    });
    if (!hasNotes) html += `<p style="color:#8aaa82">No therapist notes recorded this week.</p>`;

    html += `<h2>Patient Notes</h2>`;
    let hasPatientNotes = false;
    schedule.forEach(s => {
      const d = wdata[s.day];
      if (d?.notes) {
        html += `<p><strong>${s.day}:</strong></p><div class="notes">${d.notes}</div>`;
        hasPatientNotes = true;
      }
    });
    if (!hasPatientNotes) html += `<p style="color:#8aaa82">No patient notes recorded this week.</p>`;

    html += `<p style="margin-top:40px;font-size:0.75rem;color:#8aaa82">Generated by Rehab Planner · ${new Date().toLocaleString()}</p></body></html>`;
    ...
    if (!w) {
      const a = document.createElement('a');
      a.href = url; a.download = `rehab-report-${new Date().toISOString().slice(0,10)}.html`; a.click();
    }
  }
